...
def task_init():
    send_data_control_servo2 = bytes.fromhex('77 68 06 00 02 0B 03 10 37 0A')
    serial.write(send_data_control_servo2)
    send_data_control_servo3 = bytes.fromhex('77 68 06 00 02 0B 04 20 35 0A')  # 30
    serial.write(send_data_control_servo3)
    send_data_control_servo4 = bytes.fromhex('77 68 06 00 02 0B 05 20 50 0A')
    serial.write(send_data_control_servo4)
    send_data_control_servo_city1 = bytes.fromhex('77 68 06 00 02 0B 06 50 B4 0A')  # 90度
    ...
    send_data_control_servo1 = bytes.fromhex('77 68 06 00 02 0B 02 30 00 0A')  # 0度   B4 180度
    serial.write(send_data_control_servo1)
...
#        time.sleep(2)
        driver.stop()
        send_data_control_servo2 = bytes.fromhex('77 68 06 00 02 0B 03 20 8C 0A')
  # 6E
        serial.write(send_data_control_servo2)
        time.sleep(3)
        send_data_control_servo2 = bytes.fromhex('77 68 06 00 02 0B 03 20 37 0A')   # 高一点,
  # 37
        serial.write(send_data_control_servo2)
#        time.sleep(0.5)
...
#            serial.write(send_data_control_servo3)
#            time.sleep(1)
            send_data_control_servo4 = bytes.fromhex('77 68 06 00 02 0B 04 20 0A 0A')
            serial.write(send_data_control_servo4)
            time.sleep(1)
            send_data_control_servo3 = bytes.fromhex('77 68 06 00 02 0B 05 20 B4 0A ')  # B4
            serial.write(send_data_control_servo3)
            send_data_control_servo3 = bytes.fromhex('77 68 06 00 02 0B 04 20 35 0A')  # 30
            serial.write(send_data_control_servo3)
#            send_data_control_servo3 = bytes.fromhex('77 68 06 00 02 0B 04 20 69 0A')  # 105
#            serial.write(send_data_control_servo3)
#            time.sleep(1)
#            send_data_control_servo4 = bytes.fromhex('77 68 06 00 02 0B 05 40 B4 0A')
#            serial.write(send_data_control_servo4)
#            time.sleep(1)
...
#            time.sleep(1)
    ...
    if args == "dunhuang":  # 敦煌
        flag2 = 3.2
#        if count == 100 or count == 8:   # count == 0
#            driver.stop()
#            car.move([-10, -10, -10, -10])
#            time.sleep(2)
#            driver.stop()
        if flag1 == 3 and flag2 == 3.2:
            #            if flag_stop == 0:
            driver.stop()
            send_data_control_servo_city2 = bytes.fromhex('77 68 06 00 02 0B 07 50 5A 0A')
            serial.write(send_data_control_servo_city2)
            time.sleep(0.5)
            send_data_control_green = bytes.fromhex('77 68 08 00 02 3B 02 00 00 FF 00 0A')
            serial.write(send_data_control_green)
...
#            time.sleep(1)
            print("dingxiangjun...................................................")
            send_data_control_servo_city3 = bytes.fromhex('77 68 06 00 02 0B 08 50 5A 0A')
            serial.write(send_data_control_servo_city3)
            time.sleep(0.5)
            send_data_control_green = bytes.fromhex('77 68 08 00 02 3B 02 00 00 FF 00 0A')
            serial.write(send_data_control_green)
...
def reun_task_new(recv, args):
    ...
    if recv == "fenglangjuxu":  # 战利品 2舵机
        flag1 = 2
        send_data_control_servo1 = bytes.fromhex('77 68 06 00 02 0B 02 30 00 0A')  # 180度  转到右边
        serial.write(send_data_control_servo1)
    if args == "trophies":
        flag2 = 5
        if flag1 == 2 and flag2 == 5:
